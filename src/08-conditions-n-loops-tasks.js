...
function getCommonDirectoryPath(pathes) {
  const pathArr = Array(pathes.length);
  let result = '';
  for (let i = 0; i < pathes.length; i += 1) {
    const buff = pathes[i].split('/');
    pathArr[i] = buff;
  }
  for (let i = 0; i < pathArr.length - 1; i += 1) {
    result = '';
    for (let j = 0; j < pathArr[i].length; j += 1) {
      if (pathArr[i][j] === pathArr[i + 1][j]) result += `${pathArr[i][j]}/`;
    }
  }
...
function evaluateTicTacToePosition(position) {
  for (let col = 0; col < position[0].length; col += 1) {
    if (position[0][col] && position[0][col]
            === position[1][col] && position[1][col]
            === position[2][col]) return position[0][col];
  }
  for (let row = 0; row < position.length; row += 1) {
    if (position[row][0] && position[row][0]
            === position[row][1] && position[row][1]
            === position[row][2]) return position[row][0];
  }
  if (position[1][1]
        && ((position[0][0] === position[1][1] && position[1][1]
                === position[2][2])
            || (position[0][2] === position[1][1] && position[1][1]
                === position[2][0]))) return position[1][1];
